...
export function bigChungus(input) {
  //
...
export function drawCircle(radius, inputColor) {
...
export function drawLine(start, finish, thickness, inputColor) {
...
export function drawRect(width, height, inputColor) {
...
export function drawRhombus(axisX, axisY, inputColor) {
...
export function drawTriangle(sideLength, inputColor) {
...
export function drawPent(sideLength, inputColor) {
...
export function drawStar(sideLength, inputColor) {
  const pent = document.createElementNS(svgNameSpace, "polygon");
  const color = `hsl(${inputColor[0]}, ${inputColor[1]}%, ${inputColor[2]}%)`;
...
export function drawHex(sideLength, inputColor) {
...
export function setBg(seed, color) {
  const svg = createSvg();
...
export function selectLightness(seed, numOfSelections) {
  const colors = ["color"];
  let colorCount = 1;
  // sometimes this loop will run with no effect.  it's not count i up to num
  for (let i = 0; colors.length < numOfSelections; i += 1) {
...
export function selectHues(inputColors, seed) {
  ...
  const colors = [];
  const hues = [];
  for (let index = 0; index < inputColors.length; index += 1) {
    if (inputColors[index] === "white") {
      ...
      if (hues[0] === undefined) {
        const val = bigChungus(seed + index) % 360;
        hues.push(val);
      } else {
        // generate new hue with minimum hue contrast
        const huesNeeded = hues.length + 1;
        for (let salt = 0; hues.length < huesNeeded; salt += 1) {
...
export function selectColors(seed) {
  const lightVals = selectLightness(seed, 3);
  const colors = selectHues(lightVals, seed);
  return colors;
}

export function selectLayer(seed, dimensions, colors, colorCount, isCircle) {
  ...
  if (determiner < 0.15) {
    ...
    if (rayCount === 2) {
      ...
      // monocolor rays
      for (let i = 0; i < rayCount; i += 1) {
        const ray = drawLine(baseRay[0], baseRay[1], rayThickness, colors[1]);
        ray.setAttribute("transform", `rotate(${(i * 360) / rayCount})`);
        rotGroup.appendChild(ray);
      }
    }
...
export function selectEmblem(seed, dimensions, color, isCircle) {
  const sortedLengths = dimensions.slice().sort();
  if (isCircle) {
    sortedLengths[0] *= 0.8;
  }
  let emblemSide = sortedLengths[0];
  emblemSide *= 0.91;
...
export function selectFormat(seed, dimensions, colors, isCircle) {
...
export function createAvatar(inputID) {
